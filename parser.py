...
def extract_cases(line: str):
    regex1 = "(\\d+)年(\\d+)月(\\d+)日，(.*?)新增(\\d+)(.*?)病例(.*?)新增(\\d+)[例]{0,1}(.*?)无症状感染者.*?"
    ...
    if m3 is not None:
        (_, _, _, _, _, confirmed, _, _, asymptomatic) = m3.groups()
        return (int(confirmed), int(asymptomatic))
...
if __name__ == "__main__":
    filename = "archived_html/0ef33c367ed86680d274b71e2ce35aea.html"
    total = parse_html_to_json(filename)
    ...
    ss = ["2022年4月26日，崇明区新增14例新冠肺炎本土确诊病例，其中12例居住于闭环管理的建筑工地；新增453例无症状感染者，其中447例居住于闭环管理的建筑工地。其余分别居住于：",
          ...
          "2022年5月14日，静安区新增27本土确诊病例，新增116例无症状感染者，分别居住于："
          ]
